...
import { bookingApi, servicesApi } from "../../services/userApi";
...
function mapDJToEquipment(dj: any): Equipment & {
  ...
  const defaultImage = "https://images.unsplash.com/photo-1571330735066-03aaa9429d89?w=400&q=80";
  ...
  return {
    ...
    tag: (parseFloat(dj.ratingAverage) || 0) >= 4.8 ? "Top Pick" : (dj.ratingCount || 0) > 50 ? "Popular" : null,
  };
}
  ...
  const handleConfirm = async () => {
    ...
    try {
      await bookingApi.create({
        captainId: picked.captain?.id || 1,        // fallback to 1 if not available
                    ...
                    <TouchableOpacity style={dj.djCardRow}
                      ...
                      <View style={{ flex: 1 }}>
                        ...
                        <View style={dj.djMeta}>
                          ...
                          {item.captain?.locationCity && <View style={dj.tag}><Text style={dj.tagText}>{item.captain.locationCity}</Text></View>}
                        </View>
                      </View>
                    ...
                    {isExpanded && item.isAvailable && (
                      <View style={dj.djDetail}>
                        ...
                        <View style={dj.djStatsRow}>
                          ...
                          <View style={dj.djStatDivider} />
                          <View style={dj.djStat}><Text style={dj.djStatVal}>₹{Math.round(Number(item.hourlyRate) || 0).toLocaleString()}</Text><Text style={dj.djStatLbl}>Per Hour</Text></View>
                        </View>
                        {item.bio && <Text style={dj.djBio}>{item.bio}</Text>}
...
export default function HomeScreen() {
  ...
  const loadData = useCallback(async () => {
    try {
      // Load Featured & Popular DJs
      const djRes = await servicesApi.getAllDJs();
      const djList = djRes.success && Array.isArray(djRes.data) ? djRes.data : [];
      ...
      const bkRes = await bookingApi.getMyBookings({ limit: 3 });
                  ...
                  {loadingDJs ? (
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.featuredScroll} decelerationRate="fast" snapToInterval={CARD_WIDTH + 16}>
                      {featuredDJs.map((item) => (
                        <TouchableOpacity key={item.id} onPress={() => router.push(`/equipment/${item.id}?captainId=1` as any)} style={s.featuredCard}>
                  ...
                  <View style={s.section}>
                    ...
                    <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={s.smallCardsScroll}>
                      {popularDJs.slice(1, 5).map((item, idx) => (
                        <TouchableOpacity key={item.id} style={s.smallCard} activeOpacity={0.92} onPress={() => router.push(`/equipment/${item.id}?captainId=1` as any)}>
