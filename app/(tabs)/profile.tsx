...
import { userApi } from "../../services/userApi"; // ← Important: use userApi
  ...
  const loadStats = useCallback(async () => {
    ...
    try {
      ...
      if (profileData?.bookingStats) {
        ...
        // Fallback: count from bookings
        const bkRes = await userApi.getMyBookings();
        const list: any[] = Array.isArray(bkRes?.data) ? bkRes.data : [];
        
        const norm = (s: string) => (s ?? "").toLowerCase().trim();
        
        setStats({
          total: list.length,
          active: list.filter(b => ["pending", "confirmed", "in progress", "active", "equipment dispatched"].includes(norm(b.status))).length,
        ...
        <LinearGradient colors={["#f4f8ff", "#eef1f9", "#ffffff"]} style={{ flex: 1 }}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 48 }}>
            ...
            <View style={s.hero}>
              ...
              <TouchableOpacity style={s.editBtn} onPress={() => router.push("/profile/edit" as any)}>
            ...
            <Section title="Account">
              <MenuRow icon="person-outline" label="Full Name" sublabel={displayName} onPress={() => router.push("/profile/edit" as any)} />
              <Divider />
              <MenuRow icon="mail-outline" label="Email" sublabel={user?.email ?? "—"} onPress={() => router.push("/profile/edit" as any)} />
              <Divider />
              <MenuRow icon="call-outline" label="Phone" sublabel={user?.phone ?? "Add phone number"} onPress={() => router.push("/profile/edit" as any)} />
            </Section>
            ...
            <Section title="Preferences">
              <MenuRow icon="location-outline" label="Location" sublabel={user?.locationCity ? [user.locationCity, user.locationState].filter(Boolean).join(", ") : "Set your location"} onPress={() => router.push("/profile/edit" as any)} />
              <Divider />
            ...
            <Section title="Support">
              ...
              <Divider />
              <MenuRow icon="document-text-outline" label="Terms of Service" onPress={() => Linking.openURL("https://basswala.in/terms").catch(() => {})} />
              <Divider />
              <MenuRow icon="shield-checkmark-outline" label="Privacy Policy" onPress={() => router.push("/profile/privacy-policy" as any)} />
            </Section>
